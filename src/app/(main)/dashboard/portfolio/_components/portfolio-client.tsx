...
import { AlertTriangle, Coins, Star } from 'lucide-react'
...
export function PortfolioClient() {
  ...
  const { owned, ownedTotal, ownedValued } = data
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Portfolio Value</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Estimated value of coins in your collection.
          </p>
        </div>
        <Link
          href="/dashboard/portfolio/wishlist"
      ...
      {owned.length > 0 ? (
        <div>
          ...
          <div className="rounded-xl border border-border overflow-hidden">
            <table className="w-full">
              ...
              <tbody>
                {owned.map(item => <CoinRow key={item.id} item={item} />)}
              </tbody>
              <tfoot>
        ...
        <div className="text-center py-20 text-muted-foreground">
          ...
          <p className="text-sm">Add coins to your collection to see valuations.</p>
        </div>
      )}
