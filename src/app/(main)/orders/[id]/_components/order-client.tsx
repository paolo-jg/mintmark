...
import { Package, Shield, ExternalLink, Lock, Loader2 } from 'lucide-react'
import type { OrderStatus, TrackingStatus } from '@/types'
import FileDisputeButton from './file-dispute-button'
...
export function OrderClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!user) { router.replace(`/auth/login?redirectTo=/orders/${id}`); return }
      ...
      setOrder(data)
      setIsBuyer(data.buyer_id === user.id)
      setLoading(false)
    }
    load()
  }, [id])
                ...
                return (
                  <div key={step} className="flex items-center flex-1 last:flex-none">
                    <div className="flex flex-col items-center gap-1">
                      ...
                      <span className={`text-xs whitespace-nowrap ${done ? 'text-foreground' : 'text-muted-foreground'}`}>
      ...
      {isBuyer && isShipped && !isComplete && order.status !== 'disputed' && (
        <div className="mb-6">
          <ConfirmReceiptButton
            ...
            onConfirmed={() => setOrder((o: any) => ({ ...o, status: 'delivered' }))}
          />
        </div>
      )}
